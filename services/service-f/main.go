...
func CallMongoDB(greeting pb.Greeting) {
	...
	// client, err := mongo.Connect(ctx, options.Client().ApplyURI("mongodb://forguest:VnximZWwED@10.244.0.59:27017/admin"))
	if err != nil {
		log.Error(err)
	}
...
func GetMessages() {
	...
	//conn, err := amqp.Dial("amqp://root:VnximZWwED@10.244.0.84:5672/%2F")
	if err != nil {
		log.Error(err)
	}
	defer conn.Close()
